...
router.get('/addCard/:slug', async (req,res) => {
    const { slug } = req.params;
    req.flash('error', "Please put a todo card title!");
    res.render('create_card', { slug: slug, errors: null});
...
router.post('/createCard/:slug', async (req,res) => {
    try {
        ...
        let errors = [];
        if (!card_title) {
            errors.push({ message: "Please put a todo card title"});
        };
        if (errors.length > 0) {
            res.render('create_card', { slug: slug, errors: errors});
        };
